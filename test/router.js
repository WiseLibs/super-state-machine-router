...
describe('Router', function () {
	for (const [testSuiteType, compress] of [['compressed', true], ['uncompressed', false]]) {
		describe(testSuiteType, function () {
			...
			describe('route()', function () {
				...
				it('matches multiple non-empty segments on quantified variable segments', function () {
					...
					const router2 = new RouterBuilder().add('/{var}*', -1).build({ compress });
				...
				it('provides an array of values for quantified variables', function () {
					...
					vars = {};
					const router2 = new RouterBuilder().add('/{foobar}*', -1).build({ compress });
			...
			describe('map()', function () {
				it('throws if the callback is not a function', function () {
					expect(() => router.map()).to.throw(TypeError);
					expect(() => router.map(null)).to.throw(TypeError);
					expect(() => router.map(123)).to.throw(TypeError);
					expect(() => router.map({})).to.throw(TypeError);
					expect(() => router.map([])).to.throw(TypeError);
					expect(() => router.map(Object.create(Function.prototype))).to.throw(TypeError);
				});
				it('returns an equivalent router with new route values', function () {
					const newRouter = router.map(x => x + x);
					expect(newRouter).to.not.equal(router);
					expect(newRouter._machine).to.equal(router._machine);
					expect(newRouter._compressed).to.equal(router._compressed);
					expect(newRouter.route('/')).to.equal(2);
					expect(newRouter.route('/foo')).to.equal(4);
					const vars = {};
					expect(newRouter.route('/x/b/foo/bar/baz', vars)).to.equal(14);
					expect(vars.requiredPath).to.deep.equal(['foo', 'bar', 'baz']);
				});
			});

			describe('iterable protocol', function () {
				it('returns an iterable iterator over the router\'s values', function () {
					const values = [];
					const iterator = router[Symbol.iterator]();
					expect(iterator).to.be.a('generator');
					...
					expect(values).to.deep.equal(
						[1, 2, 3, 4, 5, 6, 7, 8, 'invalid', 'invalid', 'invalid', 'invalid', 9, 10]
					);
				});
			});
		});
